                    ...
                    if line['categories']:
                        ...
                        for category in categories:
                            ...
                            if category.replace('_', ' ') in schema_category_mappings_dict.keys():
                                pass # TODO: Decide: if we know the category has a mapping do we add it to the graph still? or just skip it?
    ...
    with open(file=file_path, mode="r") as file:
        for line in file:
            try:
                ...
                # TODO: counter object
                # TODO: Should we get all unique dates and make a blank node for those? 
                for date, count in dates:
                    
                    b_node = BNode()
                    checkin_class = get_schema_type(entity_name)
